...
def index(request):
  ...
  #   crm_host = settings.CRM_HOST
    return render_to_response('warehouse/warehouse_dashboard.html',locals(),RequestContext(request))
  # else:
  #   return HttpResponseRedirect('/login/')

@login_required(login_url='/login/')
def update_barcode(request):
    return render_to_response('warehouse/update_barcode.html',locals(),RequestContext(request))
